...
```{r basic_QC}
# plot violin plots (there is a Seurat function for this!) of number of counts per cell, number of genes per cell
...
pbmc_filtered <- subset(pbmc, subset = nFeature_RNA > ?? & nFeature_RNA < ?? & percent.mt < ??)
```
...
```{r var_features}
# Identify the 2000 most variable features in the normalised data using the FindVariableFeatures function
pbmc_filtered <- 
```
...
```{r UMAP_DimPlot}
# plot the UMAP coloured by the Louvain clustering returned above using the DimPlot function.


```

```{r PCA_clusters}
# plot the Louvain clustering onto the PCA calculated above, again using the DimPlot function. Hint: you need to specify pca as the reduction to be used here
...
# by running and plotting UMAP three times with different seeds. (the default seed is 42)

pbmc_filtered <- RunUMAP(pbmc_filtered, dims = ?:?, seed.use = 1)

DimPlot(pbmc_filtered)
```

```{r UMAP_randomness_2}
pbmc_filtered <- RunUMAP(pbmc_filtered, dims = ?:?, seed.use = ??)
...
pbmc_filtered <- FindClusters(pbmc_filtered, resolution = ?)
DimPlot(pbmc_filtered, reduction = "umap", group.by = "??")
```
...
pbmc_filtered <- SetIdent(pbmc_filtered, value = pbmc_filtered@meta.data$??)

# plot the QC metrics discussed above onto a UMAP plot
# additionally plot a UMAP plot showing the clustering resolution which you plan to proceed with
...
# first, have a look at the resulting table by plotting the first rows of the marker_genes table
...
# you might need to use the unique() function in case some marker genes appear for more than one cluster
...
Classical_Monocytes = c("gene1", "gene2")
B_cells = c("")
CD4_pos_T = c("")
CD8_pos_T = c("")
...
```{r export_matrix}
write.table(as.matrix(GetAssayData(object = pbmc_filtered, layer = "counts")), 
            'counts.csv', 
